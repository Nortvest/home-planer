...
class TestTemplatesAPI:
    ...
    def test_list_templates(self, db_path: str) -> None:
        client = _make_client(db_path)
        client.post("/api/templates", json={"title": "A", "sp_cost": 1, "recurrence_type": "none", "recurrence_params": {}})
        client.post("/api/templates", json={"title": "B", "sp_cost": 2, "recurrence_type": "none", "recurrence_params": {}})
    ...
    def test_update_template(self, db_path: str) -> None:
        ...
        tid = client.post("/api/templates", json={
            "title": "Old", "sp_cost": 1, "recurrence_type": "none", "recurrence_params": {},
        }).json()["id"]
    ...
    def test_deactivate_template(self, db_path: str) -> None:
        client = _make_client(db_path)
        tid = client.post("/api/templates", json={
            "title": "Test", "sp_cost": 1, "recurrence_type": "none", "recurrence_params": {},
        }).json()["id"]
    ...
    def test_template_negative_sp_rejected(self, db_path: str) -> None:
        client = _make_client(db_path)
        resp = client.post("/api/templates", json={
            "title": "Bad", "sp_cost": -1, "recurrence_type": "none", "recurrence_params": {},
        })
        assert resp.status_code in {400, 422}
...
class TestInstancesAPI:
    ...
    def _setup(self, db_path: str) -> tuple[TestClient, int, int, str]:
        ...
        t_resp = client.post("/api/templates", json={
            "title": "Daily Task", "sp_cost": 2, "recurrence_type": "daily",
            "recurrence_params": {}, "default_assignee_id": user_id,
        })
...
class TestCalendarAPI:
    ...
    def test_calendar_materializes_instances(self, db_path: str) -> None:
        ...
        client.post("/api/templates", json={
            "title": "Daily Ch", "sp_cost": 1, "recurrence_type": "daily",
            "recurrence_params": {}, "default_assignee_id": uid,
        })
        today = datetime.now(tz=timezone.utc).date()
        resp = client.get(f"/api/calendar?year={today.year}&month={today.month}")
        assert resp.status_code == 200
...
class TestDashboardAPI:
    ...
    def test_dashboard_shows_balance(self, db_path: str) -> None:
        ...
        t_resp = client.post("/api/templates", json={
            "title": "Task", "sp_cost": 5, "recurrence_type": "none",
            "recurrence_params": {}, "default_assignee_id": uid,
        })
        ...
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?)",
            (tpl_id, "Task", now.date().isoformat(), uid,
             now.isoformat(), uid, 5, now.isoformat()),
        )
        conn.commit()
        conn.close()
...
class TestFullLifecycle:
    ...
    def test_create_reassign_complete_balance(self, db_path: str) -> None:
        ...
        client.post("/api/templates", json={
            "title": "Мусор", "sp_cost": 3, "recurrence_type": "daily",
            "recurrence_params": {}, "default_assignee_id": u1["id"],
        })
